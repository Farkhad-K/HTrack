...
public class ExcelReportService(IHTrackDbContext context) : IExcelReportService
{
    ...
    private void BuildEmployeeSummarySheet(XLWorkbook workbook, string employeeName, List<AttendanceRow> rows)
    {
        ...
        var distinctDays = rows.Select(r => r.CheckIn.Date).Distinct().Count();
        var avgHours = distinctDays > 0 ? totalDuration.TotalHours / distinctDays : 0;
    ...
    private void BuildEmployeeDetailSheet(XLWorkbook workbook, string employeeName, List<AttendanceRow> rows)
    {
        ...
        ws.Cell(2, 1).Value = "Sana";
        ws.Cell(2, 2).Value = "Kelgan vaqti";
        ws.Cell(2, 3).Value = "Ketgan vaqti";
        ws.Cell(2, 4).Value = "Ishlagan soati";
        ...
        foreach (var a in rows)
        {
            ...
            ws.Cell(row, 1).Value = checkIn.ToString("dd.MM.yyyy", _uzCulture);
            ws.Cell(row, 2).Value = checkIn.ToString("HH:mm");
            ws.Cell(row, 3).Value = checkOut?.ToString("HH:mm") ?? "—";
            ws.Cell(row, 4).Value = $"{(int)a.Duration.TotalHours:D2}:{a.Duration.Minutes:D2}";
